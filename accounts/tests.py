...
class AccountAPIPermissionTest(AuthMixin, APITestCase):
    def setup(self):
        ...
        self.account_1 = AccountFactory(user=self.user, branch=self.branch)
        self.account_2 = AccountFactory(user=self.user, branch=self.branch)
    ...
    def test_filter_and_search(self):
        ...
        response = self.client.get(self.account_list_url, {"search": self.user.username})
